          ...
          <select class="custom-select" id="inputGroupSelect01" name="selectRole">
            <option selected>Choose...</option>
              ...
              foreach ($query->results() as $query) {
                  echo "<option " . (isset($_POST["selectRole"]) && $query->role_name == $_POST['selectRole'] ? "selected" : "") . " value='$query->role_name' ";
                  echo "> " . $query->role_name . "</option>";
              }
              ?>
          </select>
        </div>
          ...
          if (!$rawpermissions->count()) {
              ...
              foreach ($permissions->results() as $pid) {
                ...
                <td><input type="checkbox" name="checkbox[]"
                           value="<?php echo $pid->id ?>" <?php if ($pid->user_permission_id !== null) echo "checked='checked'"; ?>>
                </td>
                  <?php
